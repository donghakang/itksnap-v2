...
bool GuidedNativeImageIO::FileFormatDescriptor::TestFilename(std::string fname) {
  if (fname.length() == 0)
    return false;
  ...
  ::CreateImageIO(const char * fname, Registry & folder, bool flag_read) {
    ...
    case FORMAT_RAW: {
      ...
      case PIXELTYPE_UCHAR:
        ...
        break;
      case PIXELTYPE_CHAR:
        CreateRawImageIO < signed char > (fldRaw);
        break;
      case PIXELTYPE_USHORT:
        CreateRawImageIO < unsigned short > (fldRaw);
        break;
      case PIXELTYPE_SHORT:
...
GuidedNativeImageIO::DispatchBase *
  GuidedNativeImageIO::CreateDispatch(itk::ImageIOBase::IOComponentType comp_type) {
    // Based on the component type, read image in native mode
    switch (comp_type) {
    case itk::ImageIOBase::UCHAR:
      return new Dispatch < unsigned char > ();
      break;
    case itk::ImageIOBase::CHAR:
      return new Dispatch < signed char > ();
      break;
    case itk::ImageIOBase::USHORT:
      return new Dispatch < unsigned short > ();
      break;
    case itk::ImageIOBase::SHORT:
      return new Dispatch < short > ();
      break;
    case itk::ImageIOBase::UINT:
      return new Dispatch < unsigned int > ();
      break;
    case itk::ImageIOBase::INT:
      return new Dispatch < int > ();
      break;
    case itk::ImageIOBase::ULONG:
      return new Dispatch < unsigned long > ();
      break;
    case itk::ImageIOBase::LONG:
      return new Dispatch < long > ();
      break;
    case itk::ImageIOBase::FLOAT:
      return new Dispatch < float > ();
      break;
    case itk::ImageIOBase::DOUBLE:
      return new Dispatch < double > ();
      break;
    default:
      throw IRISException("Error: Unsupported voxel type."
...
GuidedNativeImageIO
  ::DoReadNative(const char * FileName, Registry & folder) {
    ...
    if (m_FileFormat == FORMAT_DICOM_DIR && m_DICOMFiles.size() > 1) {
      ...
      /*
       * @args m_DICOMImagesPerIPP: Number of images per z-position in the DICOM series
       */
      ...
      typename NativeImageType::SizeType dim;
      dim.Fill(1);
      typename NativeImageType::PointType org;
      org.Fill(0.0);
      typename NativeImageType::SpacingType spc;
      spc.Fill(1.0);
      typename NativeImageType::DirectionType dir;
      dir.SetIdentity();
      ...
      typename NativeImageType::IndexType index = {
        {
          0,
          0,
          0
...
template < class TOutputImage >
  ...
  RescaleNativeImageToIntegralType < TOutputImage > ::operator()(
    ...
    switch (itype) {
    case itk::ImageIOBase::UCHAR:
      DoCast < unsigned char > (native);
      break;
    case itk::ImageIOBase::CHAR:
      DoCast < signed char > (native);
      break;
    case itk::ImageIOBase::USHORT:
      DoCast < unsigned short > (native);
      break;
    case itk::ImageIOBase::SHORT:
      DoCast < signed short > (native);
      break;
    case itk::ImageIOBase::UINT:
      DoCast < unsigned int > (native);
      break;
    case itk::ImageIOBase::INT:
      DoCast < signed int > (native);
      break;
    case itk::ImageIOBase::ULONG:
      DoCast < unsigned long > (native);
      break;
    case itk::ImageIOBase::LONG:
      DoCast < signed long > (native);
      break;
    case itk::ImageIOBase::FLOAT:
      DoCast < float > (native);
      break;
    case itk::ImageIOBase::DOUBLE:
      DoCast < double > (native);
      break;
    default:
...
template < typename TPixel, typename TNative >
  class RescaleScalarNativeImageToScalarFunctor {
    public:
      ...
      RescaleScalarNativeImageToScalarFunctor(double shift, double scale): m_Shift(shift), m_Scale(scale) {}

    RescaleScalarNativeImageToScalarFunctor(): m_Shift(0), m_Scale(1) {}
...
template < typename TPixel, typename TNative >
  class RescaleVectorNativeImageToScalarFunctor {
    public:
      ...
      RescaleVectorNativeImageToScalarFunctor(double shift, double scale, size_t ncomp): m_Shift(shift), m_Scale(scale), m_Components(ncomp) {}

    RescaleVectorNativeImageToScalarFunctor(): m_Shift(0), m_Scale(1), m_Components(0) {}
...
template < typename TPixel, typename TNative >
  class RescaleVectorNativeImageToVectorFunctor {
    ...
    RescaleVectorNativeImageToVectorFunctor(double shift, double scale): m_Shift(shift), m_Scale(scale) {}

    RescaleVectorNativeImageToVectorFunctor(): m_Shift(0), m_Scale(1) {}
...
template < class TOutputImage >
  template < typename TNative >
  void
RescaleNativeImageToIntegralType < TOutputImage > ::DoCast(itk::ImageBase < 3 > * native) {
  // Get the native image
  typedef itk::VectorImage < TNative, 3 > InputImageType;
...
template < class TOutputImage, class TCastFunctor >
  ...
  CastNativeImage < TOutputImage, TCastFunctor > ::operator()(GuidedNativeImageIO * nativeIO) {
    // Get the native image pointer
    itk::ImageBase < 3 > * native = nativeIO -> GetNativeImage();
...
template < class TOutputImage, class TCastFunctor >
  template < typename TNative >
  void
CastNativeImage < TOutputImage, TCastFunctor > ::DoCast(itk::ImageBase < 3 > * native) {
  // Get the native image
  typedef itk::VectorImage < TNative, 3 > InputImageType;
  typename InputImageType::Pointer input =
    reinterpret_cast < InputImageType * > (native);
...
GuidedNativeImageIO
  ::ParseDicomDirectory(const std::string & dir, itk::Command * progressCommand) {
    ...
    const & filenames = dirList.GetFilenames();
    for (gdcm::Directory::FilenamesType::const_iterator it = filenames.begin(); it != filenames.end(); ++it) {
      ...
      bool read = false;
      try {
        read = reader.ReadSelectedTags(tags_all, true);
      } catch (...) {}
      ...
      DicomDirectoryParseResult::DicomSeriesInfo & series_info = m_LastDicomParseResult.SeriesMap[full_id];
