...
::DefaultFitContrast() {
  ...
  float t1 = (max - irange[0]) / (irange[1] - irange[0]);
  float t0 = (min - irange[0]) / (irange[1] - irange[0]);
...
{
  // template<class TWrapperTraits>
  // IntensityCurveInterface * CachingCurveAndColorMapDisplayMappingPolicy<TWrapperTraits>
  ...
  // std::cout << "(DisplayMappingPolicy.cxx) " << range[0] << ", " << range[1] << "            " << t0 << ", " << t1 << std::endl;
  // Map the extents of the control points to image units
  Vector2d outRange;
  outRange[0] = range[0] * (1 - t0) + range[1] * t0;      // minimum value
  outRange[1] = range[0] * (1 - t1) + range[1] * t1;      // maximum value
