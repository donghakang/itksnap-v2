    ...
    m_Model(NULL)
{
  ...
  m_DockLeft->setWindowTitle("LAMIS : ITK-SNAP");
...
void MainImageWindow::Initialize(GlobalUIModel *model)
{
  ...
  activateOnFlag(ui->actionLoad_multiple_from_Image, m_Model, UIF_IRIS_WITH_BASEIMG_LOADED);
...
void MainImageWindow::on_actionLoad_multiple_from_Image_triggered()
{
  // Prompt for unsaved changes
  if(!SaveModifiedLayersDialog::PromptForUnsavedSegmentationChanges(m_Model))
    return;
...
{
  std::cout << "MainImageWindox.cxx::line=1602 --> on action pressed" << std::endl;
  // Prompt for unsaved changes
  if(!SaveModifiedLayersDialog::PromptForUnsavedChanges(m_Model))
    return;
